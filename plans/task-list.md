...
- [x] Bootstrap Node.js Express server with health checking routes (`api-gateway/app.js`) - **Member 4**

## Phase 2: Core Mock Services Development (Days 4–10)
- [x] Develop CMS SOAP + REST API flask app with Spyne and PostgreSQL connectors (`cms-service/app.py`) - **Member 1**
- [x] Write logic to publish `ORDER_CREATED` RabbitMQ message in CMS service - **Member 1**
- [x] Implement ROS Node.js optimization service with Haversine greedy nearest-neighbor algorithm (`ros-service/app.js`) - **Member 2**
- [x] Implement RabbitMQ event consumer in ROS to add route stops and trigger optimization - **Member 2**
- [x] Build WMS Python TCP Socket Server (Port 9000) with concurrent threads (`wms-service/app.py`) - **Member 3**
- [x] Build WMS REST endpoints & RabbitMQ event consumer to auto-allocate packages in postgres - **Member 3**

## Phase 3: Middleware Integration & Gateway Logic (Days 11–15)
- [x] Implement Gateway REST-to-SOAP translation adapter logic for CMS - **Member 4**
- [x] Implement Gateway REST-to-TCP socket bridge logic (Port 9000 client connection) for WMS - **Member 4**
...
- [x] Execute full system container build with Docker Compose - **All Members**
- [x] Validate complete end-to-end order flow (Create -> Warehouse processing -> Driver signature -> Complete) - **All Members**
- [x] Implement and test fallback/compensation triggers for transaction errors - **All Members**
